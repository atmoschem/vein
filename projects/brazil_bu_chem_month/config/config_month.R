...
dir.create(path = paste0("post/", month), showWarnings = FALSE)
dir.create(path = paste0("post/", month, "/datatable"), showWarnings = FALSE)
dir.create(path = paste0("post/", month, "/streets"), showWarnings = FALSE)
dir.create(path = paste0("post/", month, "/grids"), showWarnings = FALSE)
...
png("images/FUEL.png", width = 2500, height = 2000, units = "px", res = 300)
dff <- data.frame(
  D = fuel[fuel$fuel == "D", ]$consumption_lt,
  E = fuel[fuel$fuel == "E", ]$consumption_lt,
  G = fuel[fuel$fuel == "G", ]$consumption_lt
)

colplot(df = dff, 
        xlab = "Months", 
        main = "Fuel Consumption (lt)")
...
# month ####
if("month" %in% names(fuel)){
  
fuel <- fuel[fuel$month == month,
             c("fuel", "density_tm3", "consumption_lt")]
row.names(fuel) <- 1:nrow(fuel)
fuel
}

met <- met[met$month == month, ]
...
saveRDS(standard, "config/standard.rds")
...
vein_notes(
  ...
  file = "notes/README",
  title = paste0("Metropolitan Area of São Paulo (MASP) ", year, "/", month),
  approach = "Bottom-up",
  traffic = "Samples of travel demand models for MASP",
  composition = "CETESB",
  ef = paste0("CETESB ", scale, ". Improved SO2 EF. 
                    Fuel consumption from state, aproximated to MASP proportions"),
