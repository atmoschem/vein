...
fleet <- rbind(frota_pasada[, names(dxr), with = F],
                   dxr[type == "original"],
                   dxr[type == "logistic" &
                         y > 2022])

names(fleet) <- toupper(names(fleet))
fleet$Year <- fleet$Y
...
df_futuro_m <- rbindlist(lapply(seq_along(yfuturo),
                 function(i) {
m[Year == 2018]
                 }))
df_futuro_m$Year <- 2019:2100

ypasado <- 1939:1983

df_pasado_m <- rbindlist(lapply(seq_along(ypasado),
                                function(i) {
                                  m[Year == 1984]
                                }))
df_pasado_m$Year <- 1939:1983


m <- rbind(df_futuro_m, m, df_pasado_m)
...
fleetx[[umm[i]]] <- ifelse(fleet$Year < 1984,
                           mmm[Year == 1984][[umm[i]]],
...
for(j in seq_along(v)) {
	...
	vd <- grep(v[j], uv, value = T)

for(i in seq_along(vd)) {

	fleetx[[vd[i]]] <- fleetx[[v[j]]]*fleetx[[paste0("p", vd)[i]]]

}
}

fleetx <- fleetx[, c("Year", "TYPE", uv, "region", "UF"), with = F]
...
dt <- melt(fleetx, id.vars = c("region", "UF", "Year", "TYPE"))
...
png(filename = paste0("figs/fleet/intial_fleet_", ur[i], ".png"),
width = 3000,
height = 2000,
res = 300
)
