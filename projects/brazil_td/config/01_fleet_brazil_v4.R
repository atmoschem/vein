...
x <- readxl::excel_sheets("masp2020/inventory.xlsx")

v <- readxl::read_excel("masp2020/inventory.xlsx",
                        "fleet_age")
setDT(v)
...
veh <- readxl::read_excel("masp2020/inventory.xlsx",
			  "metadata")$vehicles
...
png(filename = "figs/fleet_intial.png",
width = 3000,
height = 2000,
res = 300
)
...
dcast.data.table(data = dx,
                 formula = Year + type ~ veh) -> dff

rbindlist(lapply(seq_along(fn$UF),
                 function(i){
