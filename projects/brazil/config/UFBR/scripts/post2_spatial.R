...
# for grid! ####
roads <- roads[, c("length", "lengthHDV")]
roads$length <- roads$length/sum(roads$length)
roads$lengthHDV <- roads$lengthHDV/sum(roads$lengthHDV)
...
switch(language,
       ...
       "spanish" = cat("Distribuyendo los largos en las grilla\n")
)
