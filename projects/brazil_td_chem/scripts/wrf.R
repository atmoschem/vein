...
tfsall <- tfs$PC_G / sum(tfs$PC_G)
tfsnox <- tfs$TRUCKS_SL_D / sum(tfs$TRUCKS_SL_D)

for (i in 1:length(na)) {
  print(na[i])

  if (na[i] %in% c("E_NO", "E_NO2", "E_SO2")) {
    ftfs <- tfsnox
  } else {
    ftfs <- tfsall
  }

  x <- st_set_geometry(readRDS(lf[i]), NULL) / days
  x$id <- NULL
  x <- as.data.frame(as.matrix(x) %*% ftfs)
...
a <- a[[1]]
a[] <- ifelse(a[] <= 0, NA, a[])
print(sp::spplot(a, col.regions = cpt(rev = T), main = names(a), scales = list(Draw = T)))
dev.off()
