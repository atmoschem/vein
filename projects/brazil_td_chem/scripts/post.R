...
# df
years <- 2018
df <- rbind(fread("emi/exhaust.csv"), 
...
dtev
DT <- data.table(month = c(dt$month, dtev$month),
...
roads$lengthHDV <- ifelse(roads[[osm_name]] %in% c("tertiary", 
                                                "secondary", 
                                                "primary"), 0,
                       roads$length)
...
names_nox <- c(grep("NO_", DT$PFTM, value = T),
