  ...
  for (rr in seq_along(reg)) {
    ...
    for(mm in 1:12) {
      ...
      for(i in seq_along(veh_ev)) {
        ...
        dm <- rep(dm/sum(dm), 3*nrow(met[Month == mm]))
        
        mon <- rep(1:12, 3*nrow(met[Month == mm]))
        ...
        for(eva in seq_along(dd)) {
          ...
          array_x <- emis_hot_td(
            ...
            lkm = mileage[[veh_ev[i]]][1:meta_ev$maxage[i]], 
            ef = xx, 
            fortran = TRUE, 
            nt = check_nt()*0.9,
            verbose = verbose,
