...
###########################################################
###                                                    ####
###   ANNUAL EMISSIONS INVENTORY FOR ARGENTINA         ####
###                                                    ####
###########################################################
...
path <- "config/inventory_arg.xlsx"
...
# 1) Network ####
net <- st_read("network/arg_provincias.gpkg")
crs <- 4326 #(latlon epsg)
...
pol <- c( "CO", "HC", "NMHC", "NOx", "CO2", "PM", "NO2", "NO")
IM <- FALSE
source("scripts/hot_exhaust_eea.R", encoding = "UTF-8")

pol <- c("CO", "NOx", "NMHC", "HC", "NO2", "NO")
