...
for (i in seq_along(metadata_cold$vehicles)) {
  ...
  for (j in seq_along(pol)) {
    ...
    if(IM) {
      ...
      if(pol == "CO")fim <- im_co[[metadata$vehicles[i]]][1:ncol(x)]
      
      if(pol %in% c("HC", "NMHC")) fim <- im_hc[[metadata$vehicles[i]]][1:ncol(x)]
      
      if(pol %in% c("NO", "NO2", "NOx")) fim <- im_nox[[metadata$vehicles[i]]][1:ncol(x)]
      
      if(pol %in% c("PM")) fim <- im_pm[[metadata$vehicles[i]]][1:ncol(x)]
