...
for (i in seq_along(ns)) {
  ...
  df <- melt.data.table(dl, id.vars = "pol")

  names(df) <- c("pol", "veh", "ef")

  df$age <- rep(1:40, length(ln[[i]]) * length(pol))
...
for (i in seq_along(metadata$vehicles)) {
  ...
  for (j in seq_along(pol)) {
    ...
    if (IM) {
      ...
      if (pol[j] == "CO") {
        fim <- im_co[[metadata$vehicles[i]]][1:ncol(x)]
      }

      if (pol[j] %in% c("HC", "NMHC")) {
        fim <- im_hc[[metadata$vehicles[i]]][1:ncol(x)]
      }

      if (pol[j] %in% c("NO", "NO2", "NOx")) {
        fim <- im_nox[[metadata$vehicles[i]]][1:ncol(x)]
      }

      if (pol[j] %in% c("PM")) {
        fim <- im_pm[[metadata$vehicles[i]]][1:ncol(x)]
      }
    ...
    fwrite(x_DF, "emi/EXHAUST_DF.csv", append = TRUE)
    ...
    fwrite(x_STREETS, "emi/EXHAUST_STREETS.csv", append = TRUE)
  }
