...
metadata$Category_EEA <- ifelse(metadata$v_eea_old %in% c("PC", "LCV", "Motorcycle"),
  ...
  for (j in seq_along(pol)) {
    ...
    if(metadata$v_eea_old[i] %in% c("PC", "LCV", "Motorcycle")) {
      ...
      ef <- lapply(seq_along(cate), function(eu) {
        ...
        ef_eea(category = metadata$Category_EEA[i], 
               fuel = metadata$Fuel_EEA[i], 
               segment = metadata$Segment_EEA[i], 
               euro = euro[[metadata$vehicles[i]]][eu],
               tech = tech[[metadata$vehicles[i]]][eu], 
               pol = pol[j], 
               mode = metadata$Mode_EEA[i], 
               slope = 0, 
               load = 0)
      })
      ...
      ef <-       ef <- lapply(seq_along(cate), function(eu) {
