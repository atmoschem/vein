...
tf_PC <- tfs$PC_MINI_G
tf_LCV <-tfs$LCV_NI_G
tf_TRUCKS <- tfs$TRUCKS_RT_7_D
tf_BUS <- tfs$BUS_UB_15_18_D
tf_MC <- tfs$MC_2S_50_G
...
for (i in seq_along(metadata$vehicles)) {
  ...
  for (j in seq_along(pol)) {
    ...
    fwrite(emi,
           file = paste0(
             "emi/",
             metadata$vehicles[i], "/PAVED_ROADS_",
             pol[j],
             ".csv"
           )
    )
    
    g <- Emissions(colSums(emi)[1:nrow(tfs)])
    
    x_DF <- data.frame(
      array_x = rep("array_x", length(tfs[[metadata$vehicles[i]]])),
      g = g,
      veh = rep(metadata$vehicles[i], nrow(tfs)),
      size = rep("RESUS", nrow(tfs)),
      fuel = rep("ALL", nrow(tfs)),
      type_emi = rep("Paved Roads", nrow(tfs)),
      pollutant = rep(pol[j], nrow(tfs)),
      age = rep(NA, nrow(tfs)),
      hour = 1:nrow(tfs)
    )
    
    fwrite(x_DF,
           file = "emi/paved_roads.csv", 
           append = TRUE
    )
  }
}
